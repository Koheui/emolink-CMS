...
1. **注文管理機能**: 注文の一覧表示・ステータス管理・進捗確認・NFC書き込み
2. **権限管理機能**: 管理者、編集者、閲覧者の3段階の権限管理
3. **スタッフ管理機能**: スタッフの追加・削除・権限変更（管理者のみ）
4. **NFC書き込み機能**: ソニー PaSoRi RC-S300を使用したNFCタグへの書き込み

**注意**: 顧客情報の閲覧はLP側で実装済みのため、CRMでは実装不要です。
...
**注意**: 顧客管理機能（customers/）はLP側で実装済みのため、CRMでは実装しません。
...
**注意**: 顧客情報の閲覧はLP側で実装済みのため、CRMでは顧客管理機能は実装しません。

CRMでは、注文管理機能を通じて顧客情報にアクセスします（注文に紐づく顧客情報を表示）。

注文詳細ページでは、以下の顧客情報を表示します：
- メールアドレス（`orders.email`）
- 注文に関連する想い出ページ情報（`orders.memoryId`から取得）
- 公開ページURL（`claimRequests.publicPageUrl`から取得）
...
1. 注文詳細ページを開く
2. 注文から公開ページURLを取得（`claimRequests.publicPageUrl`または`orders`から）
3. 「NFC書き込み」ボタンをクリック
...
1. **Week 1**: Phase 1（権限管理の強化）✅ 完了 + PaSoRi SDK調査
2. **Week 2**: Phase 2（注文管理機能）
3. **Week 3**: Phase 3（スタッフ管理機能）
4. **Week 4**: Phase 4（NFC書き込み機能）
...
2. **データ表示テスト**: 注文情報の正確な表示（顧客情報はLP側で確認）

...
- ✅ **CRM機能の実装と改善**
  - **CRMレイアウトの実装**
    - CRM専用のレイアウトコンポーネント（`/crm/layout.tsx`）を作成
    - ヘッダーメニューに「注文管理」「顧客情報」「店舗情報」「販売数管理」「企業情報」を追加
    - 「一般ページへ」ボタンを削除（CRMとCMSは別アプリケーション）
    - ロールベースのアクセス制御（`superAdmin`, `tenantAdmin`, `editor`, `viewer`）
  
  - **注文管理ページの実装** (`/crm/orders`)
    - 注文一覧表示（注文日、メールアドレス、商品名、企業名、販売店舗名、NFC書き込みステータス）
    - 「ステータス（決済待ち）」カラムを削除
    - ソート機能（企業名、店舗名、NFC書き込みステータス）
    - 注文詳細ページ（既存の内容を維持）
    - 注文印刷ページの改善（顧客情報の優先順位を調整）
  
  - **顧客情報ページの実装** (`/crm/customers`)
    - 顧客一覧表示（メールアドレス、顧客名、emolinkページ件数）
    - データソースを`claimRequests`コレクションに変更（LP仕様に準拠）
    - 顧客詳細ページ（emolinkページごとの情報表示：企業名、店舗名、登録日、ステータス、タイトル、公開ページリンク）
    - Reactエラーの修正（オブジェクトの直接レンダリング問題を解決）
    - `resolveTenantIds`関数でテナントIDの解決を改善（識別可能なIDとFirestoreドキュメントIDの両方をサポート）
  
  - **企業管理ページの実装** (`/crm/companies`)
    - 企業一覧ページ（企業名、企業ID、店舗登録数、スタッフ登録数、メールアドレス、担当者名、電話番号、住所）
    - 企業詳細ページ（タブUI：店舗タグ、スタッフタグ）
    - 店舗情報表示（店舗名、店舗ID、郵便番号、配送先住所、電話番号、メールアドレス、バナーリンク）
    - スタッフ情報表示（管理者、編集者、スタッフの情報）
    - `superAdmin`のみアクセス可能
  
  - **販売数管理ページの改善** (`/crm/sales`)
    - 決済ステータスによるフィルタリングを削除（外部で決済処理を行うため）
    - 注文日（`createdAt`）ベースで集計するように変更
    - すべての注文を販売数としてカウント
    - 月次集計の改善とデバッグログの追加
  
  - **Firestoreルールの改善**
    - `orders`コレクションの`update`ルールを修正（`editor`, `tenantAdmin`, `superAdmin`が更新可能）
    - 開発環境での`null`値対応を追加
  
  - **データ取得関数の実装** (`firestore-crm.ts`)
    - `getAllCustomersFromClaimRequests`: `claimRequests`から顧客情報を取得
    - `getCompanyDetail`: 企業詳細情報と関連テナント・スタッフを取得
    - `getTenantDetailWithCounts`: テナント詳細と関連データの件数を取得
    - `getStaffByCompanyId`: 企業IDでスタッフを取得
    - `getAllOrders`: 注文一覧に企業名・店舗名を含める
    - `resolveTenantIds`: テナントIDの解決（識別可能なIDとFirestoreドキュメントIDの両方をサポート）
...
#### CRM機能（新規実装）
- ✅ **CRMレイアウト** (`/crm/layout.tsx`)
  - CRM専用レイアウト、ヘッダーメニュー、ロールベースアクセス制御

- ✅ **注文管理ページ** (`/crm/orders`)
  - 注文一覧表示（注文日、メール、商品名、企業名、店舗名、NFC書き込みステータス）
  - ソート機能（企業名、店舗名、NFC書き込み）
  - 注文詳細・印刷ページ

- ✅ **顧客情報ページ** (`/crm/customers`)
  - 顧客一覧表示（メール、顧客名、emolinkページ件数）
  - 顧客詳細ページ（emolinkページごとの情報表示）
  - `claimRequests`コレクションを主なデータソースとして使用

- ✅ **企業管理ページ** (`/crm/companies`)
  - 企業一覧表示（企業名、ID、店舗数、スタッフ数、連絡先情報）
  - 企業詳細ページ（店舗タグ、スタッフタグ）
  - `superAdmin`のみアクセス可能

- ✅ **販売数管理ページ** (`/crm/sales`)
  - 月次販売数の集計と表示
  - 注文日ベースでの集計（決済ステータスによるフィルタリングなし）
  - CSVエクスポート機能
...
### CRMページ（新規）
- `/src/app/crm/layout.tsx` - CRMレイアウト（ヘッダーメニュー、アクセス制御）
- `/src/app/crm/orders/page.tsx` - CRM注文管理一覧
- `/src/app/crm/orders/[id]/page.tsx` - CRM注文詳細
- `/src/app/crm/orders/[id]/print/page.tsx` - CRM注文印刷
- `/src/app/crm/customers/page.tsx` - CRM顧客情報一覧
- `/src/app/crm/customers/[id]/page.tsx` - CRM顧客詳細
- `/src/app/crm/companies/page.tsx` - CRM企業一覧（superAdmin専用）
- `/src/app/crm/companies/[id]/page.tsx` - CRM企業詳細
- `/src/app/crm/sales/page.tsx` - CRM販売数管理

### 主要なライブラリ
- `/src/lib/firestore.ts` - Firestore CRUD操作（`updateOrder()`は無効化済み）
- `/src/lib/firestore-crm.ts` - CRM用Firestoreデータ取得関数（新規）
...
#### 作業内容
- **CRM機能の実装**
  - CRMレイアウトの作成（ヘッダーメニュー、ロールベースアクセス制御）
  - 注文管理ページ（一覧、詳細、印刷）
  - 顧客情報ページ（一覧、詳細、claimRequestsベース）
  - 企業管理ページ（一覧、詳細、店舗・スタッフタブ）
  - 販売数管理ページの改善（決済ステータスフィルタリング削除）
  
- **データ取得関数の実装**
  - `firestore-crm.ts`にCRM用のデータ取得関数を追加
  - `resolveTenantIds`関数でテナントID解決を改善
  - `getAllCustomersFromClaimRequests`でLP仕様に準拠した顧客情報取得
  
- **バグ修正**
  - 顧客詳細ページのReactエラー修正（オブジェクトの直接レンダリング問題）
  - Firestoreルールの改善（ordersコレクションのupdate権限）
  - 注文印刷ページの顧客情報優先順位調整

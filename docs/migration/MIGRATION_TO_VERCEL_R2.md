...
| サービス | 役割 | 使用箇所 | コスト |
|---------|------|---------|--------|
| **Vercel** | Next.jsアプリのホスティング・CDN配信 | アプリ全体、APIルート（`/api/*`）、静的ファイル | 無料プランあり（帯域幅100GB/月、ビルド6000分/月） |
| **Cloudflare R2** | ファイルストレージ | 画像・動画・音声ファイルの保存と配信 | $0.015/GB/月（ストレージ）、ダウンロード無料 |
| **Firebase Firestore** | データベース | ユーザー情報、メモリデータ、注文情報など | 継続使用 |
| **Firebase Functions** | バックエンド処理 | Stripe Webhook、メール送信、クレーム処理 | 継続使用 |
| **Firebase Auth** | ユーザー認証 | ログイン・ログアウト、メールリンク認証、セッション管理 | 継続使用 |

### サービス別の使い分け詳細

#### 1. **Vercel** - Next.jsアプリのホスティング

- **役割**: Next.jsアプリケーションのホスティングとCDN配信、APIルートの実行環境
- **使用箇所**: 
  - Next.jsアプリ全体のホスティング
...
- **役割**: 画像、動画、音声ファイルなどの静的アセットの保存と配信
- **使用箇所**: 
  - ユーザーがアップロードするメディアファイル
  - 公開ページで表示されるコンテンツ
  - アクリルスタンド用の写真
- **メリット**: 
  - コスト削減（Firebase Storageの約42%安価）
  - ダウンロードが無料
  - S3互換APIによる高い互換性
...
- **継続使用の理由**: 既存実装、リアルタイム同期、Firebase Authとの統合

#### 4. **Firebase Functions** - バックエンド処理

- **役割**: サーバーサイドの処理実行
- **使用箇所**: 
  - Stripe Webhook（決済通知処理）
  - メール送信（`sendLoginEmail`関数）
  - クレーム処理（`claimSetUrls`関数）
- **継続使用の理由**: 既存実装、Firebase統合、セキュアな処理環境

#### 5. **Firebase Auth** - ユーザー認証

- **役割**: ユーザーの認証・認可、セッション管理
- **使用箇所**: 
  - ユーザーログイン・ログアウト
  - メールリンク認証（`/claim`ページ）
  - セッション管理
- **継続使用の理由**: 既存実装、Firestoreとの統合、セキュアな認証機能
...
詳細な使い分けについては、[FIREBASE_TO_VERCEL_MIGRATION.md](./FIREBASE_TO_VERCEL_MIGRATION.md)の「🎯 サービス別の使い分け」セクションを参照してください。

...
#### 7. トップページの簡素化
- 編集機能をすべて削除し、シンプルなランディングページに変更
- デザインをエディットページのデフォルト（ダークテーマ、緑のアクセント）に統一
- 背景色: `#000f24`、テキスト色: `#ffffff`、アクセントカラー: `#08af86`
- タイトルサイズ: 35px、本文サイズ: 16px
- Goボタンを下から15%の位置に配置（`bottom-[15%]`）
- バージョン情報（v.1.0.0）を左下に表示
- **メール認証の削除**: トップページからメール認証ページ（`/auth`）への遷移を削除。Goボタンは常に`/memories/create`に直接遷移するように変更。メール認証はLP経由（`/claim`経由）でのみ行う仕様に統一
...
- トップページからメール認証ページ（`/auth`）への遷移を完全に削除
- Goボタンは常に`/memories/create`に直接遷移し、認証チェックは`/memories/create`で実行
...
#### 1. メモリー管理画面の改善
- **検索機能**: タイトルでの検索機能を実装
- **フィルタ機能**: ステータス（すべて、公開済み、下書き）とタイプ（すべて、個人、家族、ビジネス）でのフィルタリング
- **バルク操作機能**: 
  - 選択モード（複数選択）
  - 全選択/全解除
  - 一括公開
...
- **クイック編集機能**: 
  ...
  - 各項目に削除ボタン
  - 編集ボタンで詳細編集ページへ遷移
- **結果数表示**: フィルタリング後の件数と全体件数を表示

#### 2. アルバム機能の完成
- ✅ アルバム内の写真の削除機能（実装済み）
- ✅ アルバム内の写真の順序変更機能（実装済み）

#### 3. その他の実装済み機能
- ✅ ユーザー管理画面（`/admin/users`）
- ✅ テナント管理画面（`/admin/tenants`）
- ✅ 注文管理画面（`/orders`）
- ✅ 想い出ページ作成機能（`/memories/create`）
- ✅ 画像・動画アップロード機能（Firebase Storage）
- ✅ クレーム処理機能（`/claim`）
- ✅ 公開ページ表示機能（`/public/[pageId]`）
- ✅ Originベースのテナント検証
- ✅ テナント間のデータ分離
- ✅ reCAPTCHA検証機能
